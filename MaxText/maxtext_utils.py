...
def get_abstract_state(model, tx, config, rng, mesh, is_training=True):
  ...
  if is_training and config.shard_optimizer_over_data:
    state_mesh_shardings = state_mesh_shardings.replace(
      opt_state=jax.tree.map_with_path(functools.partial(add_data_to_sharding, mesh), max_utils.unbox_logicallypartioned(abstract_state).opt_state, state_mesh_shardings.opt_state)
    )
