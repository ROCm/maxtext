      ...
      if cfg.scan_layers:
        ...
        if cfg.decoder_block == DecoderBlockType.DEEPSEEK:
          ...
          for index in range(cfg.first_num_dense_layers):
            ...
            y = dense_layer(config=cfg, mesh=mesh, name=f"dense_layers_{index}", quant=self.quant)(
                y,
                decoder_segment_ids,
                decoder_positions,
                deterministic,
          ...
          for index in range(cfg.first_num_dense_layers, cfg.num_decoder_layers):
            ...
            y = moe_layer(config=cfg, mesh=mesh, name=f"moe_layers_{index - cfg.first_num_dense_layers}", quant=self.quant)(
                y,
                decoder_segment_ids,
                decoder_positions,
                deterministic,
          ...
          for lyr in range(cfg.num_decoder_layers):
            ...
            layer = layer_ctor(config=cfg, mesh=mesh, name=f"layers_{lyr}", quant=self.quant, **layer_kwargs)
            y = layer(
                y,
                decoder_segment_ids,
                decoder_positions,
