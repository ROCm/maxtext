...
ARG BASEIMAGE=ghcr.io/rocm/jax-dev-ubu24.therock-7.14.0rc3:latest
FROM $BASEIMAGE
...
# jax-rocm pjrt/plugin (TheRock 7.14.0rc3 prerelease); overridable at build time. Must match the
# base image's ROCm SDK build (rc3). Resolved from the multi-arch prerelease index.
ARG ROCM_JAX_INDEX=https://rocm.prereleases.amd.com/whl-multi-arch/
ARG ROCM_JAX_VERSION=0.10.0+rocm7.14.0rc3
...
#   3. jax-rocm pjrt + plugin from the TheRock rc3 prerelease wheels
