...
# Dedup set: warn once per (scope, K) when an in-scope FP4 GEMM silently falls
# back to bf16 because K is not a multiple of 64 (AITER a4w4 alignment need).
_FP4_KALIGN_FALLBACK_WARNED = set()
...
class AiterFp4DotGeneralOp(nn.Module):
  ...
  def __call__(self, inputs, kernel, dimension_numbers, precision=None, **kwargs):
    ...
    # Default-ON MLP gate. Set AITER_FP4_MLP=0 to keep the MLP GEMMs in bf16 for
    # an attn-only-FP4 isolation A/B (complement of mlp-only). With the default
    # (=1) this is byte-identical to the prior "MLP always FP4" behavior.
    fp4_mlp = os.environ.get("AITER_FP4_MLP", "1") == "1"
    ...
    fp4_in_scope = (
        ((is_mlp and fp4_mlp) or ((not is_mlp) and fp4_attn)) and not force_bf16_depth
    )
    use_fp4 = (K % 64 == 0) and fp4_in_scope
    # Footgun guard: an in-scope FP4 GEMM whose contraction dim K is not a
    # multiple of 64 SILENTLY runs bf16 (the a4w4 kernel needs K%64==0). Warn
    # once per (scope, K) so a new model/config never loses FP4 unnoticed.
    if fp4_in_scope and (K % 64 != 0):
      _kalign_key = ("mlp" if is_mlp else "attn", K)
      if _kalign_key not in _FP4_KALIGN_FALLBACK_WARNED:
        _FP4_KALIGN_FALLBACK_WARNED.add(_kalign_key)
        print(
            f"[AITER_FP4] WARNING: {'MLP' if is_mlp else 'attn'} GEMM has K={K} "
            f"(not a multiple of 64); MXFP4 a4w4 requires K%64==0, so this GEMM "
            f"SILENTLY FALLS BACK to bf16 (module={self.module_context}). Set "
            f"AITER_FP4_LOG_DEPTH=1 for the full per-GEMM breakdown."
        )
