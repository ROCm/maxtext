"""Correctness check: Mixtral 8x7B baseline (stock MaxText) vs FlyDSL backend.

Runs one prefill of a fixed prompt through each backend and compares the
resulting logits. The FLY run uses the production integration path
(``fly_moe_backend``) -- no manual class patching. Each backend builds a fresh
engine and loads params with the SAME RNG seed, so both see identical random
wi_0/wi_1/wo; any logit difference is purely the FlyDSL kernel vs ragged_dot.
...
Usage::
    ...
    python3 -m maxtext.integration.flydsl.verify_correctness_mixtral \\
...
def _build_config(maxtext_dir: str, seq: int, out_dir: str, moe_backend: str = "default"):
    ...
    tokenizer = f"{maxtext_dir}/src/maxtext/assets/tokenizers/tokenizer.mistral-v1"
    overrides = [
...
def _run_backend(config, seq: int, rng_seed: int, fly: bool) -> np.ndarray:
    ...
    ``fly_moe_backend`` (production path). Engine + params are freed before
    returning so the two backends don't co-reside in HBM.
    """
    import jax
...
def main() -> int:
    ...
    if args.out_dir is None:
        args.out_dir = f"{args.jaxflydsl_dir}/results/verify"
    ...
    print("\n" + "=" * 78)
    print("Verifying correctness: Mixtral 8x7B baseline vs FlyDSL (moe_backend=flydsl)")
    ...
    if top1_pass and abs_pass:
        ...
        print(f"    Model behavior differs. Investigate:")
        print(f"      1. Routing: gate weight read, softmax+topk renorm")
        print(f"      2. moe_sort_jax per-expert ordering")
        print(f"      3. Weight concat order (gate-then-up vs up-then-gate)")
        print(f"      4. FlyDSL kernel: try in_dtype/out_dtype variants")
        return 1
